...
class figure_model
{
    ...
    std::vector<figure::Vertex> vertices;
    std::vector<vector<float>> normals;
    std::vector<figure::Vertex> texture;
...
public:
    ...
    std::tuple<figure::Vertex*, int>  get_texture() const
    {
        auto fsize = faces[0].size();
        auto res = new figure::Vertex[faces.size() * fsize];
        for (int i = 0; i < faces.size(); i++)
    ...
    std::vector<float> get_vertices_and_normals()
    {
        auto v = std::get<0>(get_vertices());
        auto n = std::get<0>(get_normals());
